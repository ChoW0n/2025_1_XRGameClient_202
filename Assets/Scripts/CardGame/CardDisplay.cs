...
public class CardDisplay : MonoBehaviour
{
    ...
    public void SetupCard(CardData data)
    {
        ...
        {
            descriptionText.text = data.description + data.GetAddtionalEffectsDescription();
        }
    }
        ...
        if (Physics.Raycast(ray, out hit, Mathf.Infinity, enemyLayer))
        {
            ...
            if (enemyStats != null)
            {
                if (cardData.cardType == CardData.CardType.Attack)              //카드 효과에 따라
                {
                    ...
                    Debug.Log($"{cardData.cardName} 카드로 {cardData.effectAmount} 데미지를 입혔습니다.");
                    cardUsed = true;
                }
            }
            else
        ...
        foreach (var effect in cardDataCopy.additionalEffects)
        {
            switch (effect.effectType)
            {
                ...
                case CardData.AdditionalEffectType.DiscardCard:         //카드 버리기 구현 랜덤 버리기
                    for (int i = 0; i < effect.effectAmount; i++)
                    {
                        if (cardManager != null && cardManager.handCards.Count > 0)
                        {
                            ...
                            {
                                //cardIndexCopy가 더 이상 유효하지 않을 경우 아무 카드나 버림
                                cardManager.DiscardCard(randomIndex);
                            }
                        }
                    }
                ...
                case CardData.AdditionalEffectType.ReduceEnemyMana:    //적 마나 감소
                    GameObject[] enemies = GameObject.FindGameObjectsWithTag("Ebentg");     //태그를 사용하여 적 찾기
                    foreach (GameObject enemy in enemies)
                    {
                        ...
                        if (enemyStats != null)
                        {
                            ...
                            Debug.Log($"마나를 {effect.effectAmount} 획득 했습니다! (현재 마나 : {enemyStats.currentMana}");
                        }
                    }
                    break;
